...
- Mirror each container in `core_config/trait_schema.yaml` with curiosity=1.0, ucn=0, adaptive_weight:true, and sensitivity flags from this reference.
- Confirm Container Studio and trait registry surfaces these entries with correct governance tagging before production rollout.
- Run `python3 -m compileall ReDNACoreDemo ExplorerDev ExplorerFinal` to catch syntax regressions after schema edits.

Operational notes:
- Identity, Social, and Work containers backfill from legacy bundles; run `core/scripts/reconcile_cli.py --user <id>` if you need to regenerate resolved snapshots post-upgrade.
- Health, Finance, and Family containers remain WRITE_PROTECT by default—updates must go through governance-approved pipelines.
- Explorer renders 🔒 for any trait flagged sensitive or high; update `ExplorerDev/schema_utils.py` if additional governance tiers launch.
