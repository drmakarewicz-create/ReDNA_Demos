...
_PERSONA_FALLBACK_META: Dict[str, Dict[str, str]] = {
    "head coach": {
        "title": "Head Coach",
        "icon": "🧠",
        "color": "#1F2933",
    ...
    "padna": {
        ...
        "icon": "🧬",
        "color": "#7C3AED",
        "description": "Packages PaDNA traits into outbound-ready avatars.",
    },
    "photo": {
        "title": "Photo Coach",
        "icon": "📷",
...
def _ensure_bus_descriptor(canonical: str, *, force: bool = False) -> None:
    """Register a minimal persona descriptor with the UI bus when missing."""

    if canonical not in _PERSONA_DESCRIPTORS:
        return
    bus_id = _PERSONA_BUS_IDS.get(canonical)
    if not bus_id:
        return
    try:
        from ui.persona_bus import (  # type: ignore
            PersonaDescriptor as _BusDescriptor,
            get_persona as _bus_lookup,
            register_persona as _bus_register,
        )
    except Exception:
        return
    ...
    if not force:
        try:
            if _bus_lookup(bus_id) is not None:
                return
        except Exception:
            # If lookup fails, fall through to overwrite the descriptor.
            pass

    fallback_meta = _PERSONA_FALLBACK_META.get(canonical, {})
    descriptor = _PERSONA_DESCRIPTORS[canonical]
    title = fallback_meta.get("title", descriptor.label)
    icon = fallback_meta.get("icon", descriptor.icon)
    color = fallback_meta.get("color", "#1F2933")
    description = fallback_meta.get("description", "")

    try:
        payload = _BusDescriptor(
            id=bus_id,
            title=title,
            icon=icon,
            color=color,
        ...
        _bus_register(payload, overwrite=force)
    except Exception:
        return
...
def set_persona_for_user(
    ...
    *,
    ...
    if status_state == "off":
        note = f" ({status_note})" if status_note and status_note != "OK" else ""
        ss["_toast_persona_err"] = f"{descriptor.label} is disabled{note}."
        return
    ...
    if persona_set_active is not None:
        try:
            persona_set_active(bus_id, trigger=reason, metadata=payload_meta)
        except (KeyError, RuntimeError):
            _ensure_bus_descriptor(canonical, force=True)
            try:
                persona_set_active(bus_id, trigger=reason, metadata=payload_meta)
            except (KeyError, RuntimeError):
                if normalized_user and previous not in (None, canonical):
                    mapping[normalized_user] = previous
                ss["_toast_persona_err"] = f"{descriptor.label} isn't available right now."
                return
    else:
        # Even without a live persona bus we want the fallback registered so
        # subsequent attempts succeed once the bus is available.
        _ensure_bus_descriptor(canonical, force=False)
    ...
    if normalized_user and previous != canonical:
